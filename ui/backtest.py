...
def render_backtest(
    ...
    with col1:
        symbol = st.text_input(
            "Symbol", value="AAPL", help="Enter stock ticker symbol"
        ).upper()
    ...
    with col2:
        period = st.selectbox(
            "Period", ["1mo", "3mo", "6mo", "1y", "2y"], help="Historical data period"
        )
        interval = st.selectbox(
            "Interval", ["1h", "1d", "1wk"], help="Data interval/timeframe"
        )
        ...
        with st.spinner(f"Running backtest on {symbol}..."):
            try:
                ...
                if data.empty:
                    ...
                    st.write("- Invalid ticker symbol (try 'AAPL', 'MSFT', 'GOOGL')")
