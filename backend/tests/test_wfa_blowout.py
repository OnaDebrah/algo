...
from backend.app.schemas.backtest import WFARequest, ParamRange, BacktestResult
...
class TestWFABlowout(unittest.IsolatedAsyncioTestCase):
    async def test_blown_account_stops_loop(self):
        ...
        with patch('backend.app.services.walk_forward_service.fetch_stock_data') as mock_fetch:
            ...
            service._optimize_in_sample.return_value = ({"p": 1}, BacktestResult(
                total_return=0, total_return_pct=-10, win_rate=0, sharpe_ratio=0,
                max_drawdown=0, total_trades=0, winning_trades=0, losing_trades=0,
                avg_profit=0, avg_win=0, avg_loss=0, profit_factor=0, 
                final_equity=900, initial_capital=1000
            ))
            ...
            service._run_out_of_sample.side_effect = [
                (BacktestResult(
                    total_return=0, total_return_pct=-50, win_rate=0, sharpe_ratio=0,
                    max_drawdown=0, total_trades=0, winning_trades=0, losing_trades=0,
                    avg_profit=0, avg_win=0, avg_loss=0, profit_factor=0, 
                    final_equity=500, initial_capital=1000
