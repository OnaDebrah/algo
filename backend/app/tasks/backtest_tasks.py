...
# ---------------------------------------------------------------------------
# Bayesian Optimization (async impl + Celery wrapper)
# ---------------------------------------------------------------------------


async def _run_bayesian_optimization(backtest_id: int, request_data: dict, user_id: int):
    """Async implementation of Bayesian optimization task."""
    from sqlalchemy import select
    ...
    try:
        async with TaskSession() as db:
            try:
                ...
                # Reconstruct the typed request
                request = BayesianOptimizationRequest(**request_data)

                # Run optimization (this is the heavy computation)
                async with OptimizationRunner(request=request, user_id=user_id, max_workers=4) as runner:
                ...
                if not trials:
                    await _mark_failed(db, backtest_id, "All optimization trials failed. Check strategy configuration.")
                    return {"status": "failed", "backtest_id": backtest_id, "error": "All trials failed"}

                # Store results in extended_results.bayesian
                bayesian_result = {
                ...
                if run:
                    run.status = "completed"
                    run.completed_at = datetime.now(timezone.utc)
                    run.extended_results = {"bayesian": bayesian_result}
                    # Store summary metrics in standard columns for history view
                    run.sharpe_ratio = study.best_value if request.metric == "sharpe_ratio" else None
                    run.total_return_pct = study.best_value if request.metric == "total_return" else None
                    await db.commit()

                logger.info(f"Bayesian optimization {backtest_id} completed: best_value={study.best_value:.4f}")
                return {"status": "completed", "backtest_id": backtest_id}

            except SoftTimeLimitExceeded:
                logger.warning(f"Bayesian optimization {backtest_id} timed out (soft limit)")
                error_msg = "Task timed out — Bayesian optimization took too long"
                await _mark_failed(db, backtest_id, error_msg)
                return {"status": "failed", "backtest_id": backtest_id, "error": error_msg}

            except Exception as e:
                logger.error(f"Bayesian optimization {backtest_id} failed: {e}", exc_info=True)
                await _mark_failed(db, backtest_id, str(e)[:500])
                return {"status": "failed", "backtest_id": backtest_id, "error": str(e)}
    finally:
        await task_engine.dispose()
...
@celery_app.task(
    bind=True,
    name="app.tasks.backtest_tasks.run_bayesian_optimization_task",
    time_limit=900,  # 15 min hard kill (50 trials × ~5s each + overhead)
    soft_time_limit=840,  # 14 min soft limit
)
def run_bayesian_optimization_task(self, backtest_id: int, request_data: dict, user_id: int):
    """Celery task: run Bayesian optimization in background."""
    logger.info(f"[Celery] Starting Bayesian optimization {backtest_id}")
    return asyncio.run(_run_bayesian_optimization(backtest_id, request_data, user_id))
