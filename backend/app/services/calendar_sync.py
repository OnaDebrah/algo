...
RECURRING_EVENTS: List[Dict] = [
    ...
    {"name": "ADP Employment Change", "category": "Employment", "impact": "medium", "day_offset": -2,
     "anchor": "first_friday"},
    ...
    # Fed
    {"name": "FOMC Interest Rate Decision", "category": "Fed", "impact": "high",
     "months": [1, 3, 5, 6, 7, 9, 11, 12], "day": 15},
    {"name": "FOMC Meeting Minutes", "category": "Fed", "impact": "high",
     "months": [1, 2, 4, 5, 7, 8, 10, 11], "day": 20},
    {"name": "Fed Chair Press Conference", "category": "Fed", "impact": "high",
     "months": [1, 3, 5, 6, 7, 9, 11, 12], "day": 15},

    # GDP
    {"name": "GDP (QoQ) - Advance", "category": "GDP", "impact": "high",
     "months": [1, 4, 7, 10], "day": 26},
    {"name": "GDP (QoQ) - Second Estimate", "category": "GDP", "impact": "medium",
     "months": [2, 5, 8, 11], "day": 27},
    ...
    {"name": "ISM Manufacturing PMI", "category": "Manufacturing", "impact": "high",
     "day": 1},
    {"name": "ISM Services PMI", "category": "Manufacturing", "impact": "high",
     "day": 3},
    {"name": "Industrial Production (MoM)", "category": "Manufacturing", "impact": "medium",
     "day": 16},
    {"name": "Durable Goods Orders (MoM)", "category": "Manufacturing", "impact": "medium",
     "day": 25},
            ...
            if ev.get("day") == "first_friday":
                ...
                for d in range(1, max_day + 1):
                    if datetime(year, month, d).weekday() == ev["weekday"]:
                        event_dt = datetime(year, month, d, 13, 30,
                                            tzinfo=timezone.utc)
                        events.append({
                            "event_name": ev["name"],
                            "country": "US",
...
async def sync_calendar(db: AsyncSession, months_ahead: int = 6) -> int:
    ...
    await db.execute(
        delete(EconomicEvent).where(EconomicEvent.source == "generated")
    )
