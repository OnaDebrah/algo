...
class IntegratedMomentsStrategy(BaseStrategy):
    ...
    def build_model(self, input_dim: int) -> IntegratedMomentsModel:
        ...
        self.model = IntegratedMomentsModel(input_dim=input_dim, hidden_dim=128, lstm_layers=2, dropout=0.2)
        return self.model
    ...
    def prepare_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Realized moments
        features["realized_skew"] = returns_1d.rolling(20).apply(lambda x: stats.skew(x) if len(x) > 5 else 0)
        features["realized_kurt"] = returns_1d.rolling(20).apply(lambda x: stats.kurtosis(x) if len(x) > 5 else 0)
    ...
    def fit(
        ...
        # Prepare sequences
        X_seq, y_mean_seq, y_vol_seq = self.prepare_sequences(X_scaled, y_mean_scaled, y_vol_scaled)
    ...
    def generate_signal(self, data: pd.DataFrame) -> Union[int, Dict]:
        ...
        if data.empty or len(data) < self.sequence_length + 10:
            return {
                "signal": 0,
                "position_size": 0,
                "metadata": {"error": "Insufficient data", "expected_return": 0, "expected_volatility": 0.2, "conviction": 0},
            }
        ...
        try:
            ...
            return {"signal": 0, "position_size": 0, "metadata": {"error": str(e), "expected_return": 0, "expected_volatility": 0.2, "conviction": 0}}
        ...
        return {"signal": prediction.signal, "position_size": prediction.position_size, "metadata": metadata}
    ...
    def predict(self, data: pd.DataFrame) -> IntegratedPrediction:
        ...
        self.prediction_history.append({"timestamp": datetime.now(), "prediction": prediction})
