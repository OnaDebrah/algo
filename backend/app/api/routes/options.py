...
def _generate_mock_chain(symbol: str, price: float) -> ChainResponse:
    ...
    expiration_dates = [(datetime.now() + timedelta(days=d)).strftime("%Y-%m-%d") for d in [7, 14, 30, 60, 90]]
...
async def run_backtest(request: BacktestRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        try:
            ...
            data = pd.DataFrame({"Open": opens, "High": highs, "Low": lows, "Close": prices}, index=dates)
...
async def analyze_strategy(
    request: StrategyAnalysisRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        for leg in request.legs:
            ...
            builder.add_leg(
                option_type=option_type, strike=leg.strike, expiry=expiry, quantity=leg.quantity, premium=leg.premium, volatility=request.volatility
            )
        ...
        payoff_diagram = [PayoffPoint(price=float(p), payoff=float(pf)) for p, pf in zip(price_range, payoffs)]
...
async def calculate_greeks(request: GreeksRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
...
async def compare_strategies(
    request: StrategyComparisonRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
        ...
        for strategy_config in request.strategies:
            ...
            if strategy_type and hasattr(OptionsStrategy, strategy_type.upper()):
                ...
                for leg in legs:
                    ...
                    builder.add_leg(
                        option_type=option_type, strike=leg["strike"], expiry=expiry, quantity=leg["quantity"], premium=leg.get("premium")
                    )
...
async def calculate_probability(
    request: ProbabilityRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        prob_touch = OptionsAnalytics.calculate_probability_touch(
            current_price=request.current_price, barrier=request.strike, days_to_expiration=request.days_to_expiration, volatility=request.volatility
        )
        ...
        intrinsic = (
            max(request.current_price - request.strike, 0) if request.option_type == "call" else max(request.strike - request.current_price, 0)
        )
...
async def optimize_strike(
    ...
    try:
        ...
        strikes = [StrikeAnalysis(**analysis) for analysis in strike_analysis]

        return StrikeOptimizerResponse(
            symbol=request.symbol, strategy_type=request.strategy_type, current_price=request.current_price, strikes=strikes
        )
...
async def calculate_risk_metrics(
    request: RiskMetricsRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        var = OptionsAnalytics.calculate_var(
            portfolio_value=request.portfolio_value, returns=returns_array, confidence_level=request.confidence_level
        )
        ...
        if len(returns_array) > 0:
            ...
            if len(wins) > 0 and len(losses) > 0:
                ...
                kelly_fraction = OptionsAnalytics.calculate_kelly_criterion(win_prob=win_prob, avg_win=avg_win, avg_loss=avg_loss)
...
async def calculate_portfolio_stats(
    request: PortfolioStatsRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
...
async def run_monte_carlo(request: MonteCarloRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
