...
def map_strategy_to_frontend(rec: StrategyRecommendation, idx: int, request: GuideRequest) -> Recommendation:
    ...
    time_map = {"low": "5-15 min/day", "medium": "15-30 min/day", "high": "30-60 min/day"}
...
async def generate_guide(request: GuideRequest, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)):
    """
    ...
    recommendations = [map_strategy_to_frontend(rec, idx, request) for idx, rec in enumerate(core_recommendations)]
    ...
    return GuideResponse(recommendations=recommendations, radar_data=radar_data)
