    ...
    def execute_trade(self, symbol: str, signal: int, current_price: float, timestamp):
        ...
        if signal == 1 and self.position is None:
            ...
            if total_cost <= self.cash:
                ...
                logger.info(
                    f"BUY: {quantity} {symbol} @ ${slipped_price:.2f} "
                    f"(Cost: ${total_cost:.2f}, Comm: ${commission:.2f})"
                )
        ...
        # Sell signal
            ...
            profit_pct = (
                profit / (self.position["entry_price"] * self.position["quantity"])
            ) * 100
