...
def render_options_backtest(db: DatabaseManager, risk_manager: RiskManager):
    ...
    with col2:
        ...
        if st.button(
            "Vertical Call Spread", key="bt_call_spread", use_container_width=True
        ):
            st.session_state["selected_strategy"] = OptionsStrategy.VERTICAL_CALL_SPREAD
        if st.button(
            "Vertical Put Spread", key="bt_put_spread", use_container_width=True
        ):
    ...
    with col3:
        ...
        if st.button(
            "Protective Put", key="bt_protective_put", use_container_width=True
        ):
            st.session_state["selected_strategy"] = OptionsStrategy.PROTECTIVE_PUT

    # Display selected strategy configuration
    if (
        "selected_strategy" in st.session_state
        and st.session_state["selected_strategy"] is not None
    ):
        st.markdown("---")
        _render_backtest_configuration(
            st.session_state["selected_strategy"], db, risk_manager
        )
    else:
        st.info("👆 Select a strategy above to begin backtesting")


def _render_backtest_configuration(
    strategy: OptionsStrategy, db: DatabaseManager, risk_manager: RiskManager
):
    ...
    with col1:
        symbol = st.text_input(
            "Underlying Symbol", value="SPY", help="Stock ticker to backtest"
        ).upper()
    ...
    with col3:
        interval = st.selectbox(
            "Data Interval", ["1d", "1wk"], index=0, help="Data granularity"
        )
    ...
    with col2:
        if signal_type == "regular":
            entry_frequency = st.number_input(
                "Entry Frequency (days)",
                min_value=1,
                max_value=90,
                value=30,
                help="Days between new positions",
            )
        else:
            entry_frequency = 30
    ...
    with col1:
        profit_target = (
            st.slider(
                ...
                max_value=200,
                value=50,
                help="Close position at this % profit",
            )
            / 100
        )
    ...
    with col2:
        loss_limit = (
            st.slider(
                ...
                max_value=200,
                value=50,
                help="Close position at this % loss",
            )
            / 100
        )

    with col3:
        dte_exit = st.number_input(
            "DTE Exit",
            min_value=1,
            max_value=30,
            value=7,
            help="Close position with days remaining",
        )
    ...
    with col2:
        volatility = st.slider(
            ...
            step=0.05,
            format="%.0f%%",
            help="Expected volatility for pricing",
        )
...
def _display_backtest_results(results: dict, symbol: str, strategy: OptionsStrategy):
    ...
    with col1:
        st.metric(
            ...
            delta=f"${results['final_equity'] - results.get('initial_capital', 100000):,.0f}",
        )
    ...
    with col2:
        st.metric(
            ...
            delta=f"{results['winning_trades']}/{results['total_trades']} wins",
        )
    ...
    with col3:
        st.metric(
            ...
            delta="Good" if results["profit_factor"] > 1.5 else "Needs Improvement",
        )
...
def _display_trade_analysis(closed_positions: list):
    ...
    with col1:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Histogram(x=df["pnl"], nbinsx=30, marker_color="#00ff88", opacity=0.7)
        )
    ...
    with col2:
        ...
        fig.add_trace(
            go.Histogram(
                x=df["pnl_pct"] * 100, nbinsx=30, marker_color="cyan", opacity=0.7
            )
        )
