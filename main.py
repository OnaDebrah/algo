...
def render_top_ticker():
    """Fetches and displays a global market ribbon at the top of the app"""
    ...
    # Fetch data (cached for 2 minutes to prevent API throttling)
    ...
    def fetch_ticker_data():
        ...
        for sym, name in symbols.items():
            try:
                ...
                change = ((price - info.open) / info.open) * 100
    ...
    for i, item in enumerate(ticker_items):
        ...
        with cols[i]:
            st.markdown(
                f"""
                <div style="background-color: rgba(255,255,255,0.03); padding: 5px 10px; border-radius: 5px; border-left: 3px solid {color};">
                    <p style="margin:0; font-size: 0.7rem; color: #9AA4B2; text-transform: uppercase;">{item['name']}</p>
                    <p style="margin:0; font-size: 0.9rem; font-weight: bold;">
                        {item['price']:,.2f} <span style="color: {color}; font-size: 0.8rem;">{icon} {abs(item['change']):.2f}%</span>
                    </p>
                </div>
                """,
                unsafe_allow_html=True,
