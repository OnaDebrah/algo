...
import {
    Zap, ClipboardList, Target, ShieldCheck,
    BarChart, BrainCircuit, CheckCircle2,
    RotateCcw, Info, TrendingUp, Users,
    ChevronRight, ArrowRight, Gauge,
    Star, Clock, DollarSign,
    AlertTriangle, Award, Sparkles, Download,
    Settings, Eye, BarChart3, Shield, Cpu,
    Rocket, Gem, Crown, LineChart, Filter,
    PieChart, Wallet, Target as TargetIcon,
    TrendingUp as TrendingUpIcon, Globe,
    Shield as ShieldIcon, Zap as ZapIcon,
    Flame, Coins, Diamond, Sparkle, Lock,
    BarChart4, Network, Binary, CircuitBoard
} from "lucide-react";
import {
...
const AIAdvisor = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const res = await advisor.guide(quizAnswers);
            if (res.data) {
                // Map backend response to UI format
                const recs = res.data.recommendations.map((r: any) => ({
                    ...r,
    ...
    const RiskBadge = ({ level, theme }: { level: string, theme: any }) => {
        const config = {
        ...
        return (
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase ${cfg.bg} ${cfg.border} border ${cfg.color}`}>
                <Shield size={12} />
                {level} Risk
            </span>
        );
    ...
    const PurpleButton = ({ children, icon, onClick, variant = 'primary', className = '' }: any) => {
        const variants = {
    ...
    if (view === 'quiz') {
        return (
            <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
                <div className="bg-gradient-to-br from-slate-900 via-slate-900/95 to-slate-900/90 border border-slate-800/50 rounded-2xl p-8 shadow-2xl shadow-slate-900/50 backdrop-blur-sm">
                    {/* Colorful Header */}
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10">
                        ...
                        <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-900/50 to-blue-900/50 rounded-xl border border-purple-700/30">
                    ...
                    <div className="mb-8">
                        ...
                        <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                            <div className="h-full w-1/3 bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500 rounded-full transition-all duration-500" />
                        </div>
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                {[
                                    { icon: Rocket, label: "Wealth Growth", desc: "Maximum capital appreciation", color: "from-red-500 to-orange-500" },
                                    { icon: TrendingUpIcon, label: "Income", desc: "Regular monthly returns", color: "from-emerald-500 to-teal-500" },
                                    { icon: Shield, label: "Protection", desc: "Capital preservation", color: "from-blue-500 to-indigo-500" }
                                ].map((goal, idx) => (
                                    <button
                                        key={idx}
                                    ...
                                    >
                                        <goal.icon className={`${quizAnswers.goal === goal.label ? 'text-white' : 'text-slate-500'}`} size={20} />
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex flex-col items-center gap-2">
                                                <div className={`p-2 rounded-lg ${quizAnswers.experience === exp ? `bg-gradient-to-br ${colors[idx]}` : 'bg-slate-800'}`}>
                                                    <Crown size={16} className={quizAnswers.experience === exp ? 'text-white' : 'text-slate-500'} />
                                                </div>
                                                <span className={`text-sm font-medium ${quizAnswers.experience === exp ? 'text-white' : 'text-slate-400'}`}>{exp}</span>
                                            </div>
                                        </button>
                                    );
                                })}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setQuizAnswers({ ...quizAnswers, capital: parseInt(e.target.value) })}
        ...
        return (
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700 max-w-6xl mx-auto">
                ...
                <div className={`${strategy.theme.bg} border ${strategy.theme.border} rounded-2xl p-8`}>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-lg border border-purple-500/30">
                        ...
                        <div className="text-center bg-slate-900/50 p-6 rounded-2xl border border-slate-800 min-w-[140px]">
                            <p className="text-5xl font-black bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                                {strategy.fit_score}%
                            </p>
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">AI Fit Score</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-6">
                        {strategy.tags.map((tag, idx) => (
                            <span key={idx} className="px-3 py-1.5 bg-slate-900/50 rounded-lg text-xs font-medium text-slate-300 border border-slate-800">
                                {tag}
                            </span>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { icon: TrendingUp, label: "Expected Return", value: strategy.expected_return, color: "text-emerald-400", bg: "bg-emerald-500/10" },
                        { icon: Users, label: "Similar Traders", value: strategy.similar_traders, color: "text-blue-400", bg: "bg-blue-500/10" },
                        { icon: Clock, label: "Time Commitment", value: strategy.time_commitment, color: "text-amber-400", bg: "bg-amber-500/10" },
                        { icon: Award, label: "Success Rate", value: strategy.success_rate, color: "text-purple-400", bg: "bg-purple-500/10" }
                    ].map((stat, idx) => (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* Pros & Cons */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-gradient-to-br from-emerald-900/20 to-teal-900/20 border border-emerald-800/30 rounded-2xl p-6">
                            ...
                            <div className="bg-gradient-to-br from-red-900/20 to-orange-900/20 border border-red-800/30 rounded-2xl p-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-800/30 rounded-2xl p-6">
                            ...
                            <ul className="space-y-3">
                                {strategy.best_for.map((item, idx) => (
                        ...
                        <div className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 border border-purple-800/30 rounded-2xl p-6">
                            ...
                            <ul className="space-y-3">
                                {strategy.why.map((reason, idx) => (
                                    <li key={idx} className="flex items-start gap-3">
                                        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-600/30 to-pink-600/30 flex items-center justify-center flex-shrink-0 mt-0.5">
    ...
    return (
        <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-700 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 bg-gradient-to-br from-purple-600/30 to-pink-600/30 rounded-lg border border-purple-500/30">
            ...
            <div className="bg-slate-900/50 border border-slate-800/50 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        { icon: ShieldCheck, label: 'Risk Tolerance', value: quizAnswers.risk, color: 'text-blue-400', bg: 'bg-blue-500/10' },
                        { icon: TargetIcon, label: 'Investment Goal', value: quizAnswers.goal, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                        { icon: TrendingUpIcon, label: 'Experience', value: quizAnswers.experience, color: 'text-amber-400', bg: 'bg-amber-500/10' },
                        { icon: DollarSign, label: 'Capital', value: `$${quizAnswers.capital.toLocaleString()}`, color: 'text-purple-400', bg: 'bg-purple-500/10' }
                    ].map((item, idx) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* AI Insights Panel */}
                <div className="space-y-6">
                    <div className="bg-gradient-to-br from-purple-900/30 to-pink-900/30 border border-purple-800/30 rounded-2xl p-6">
                        <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-3 bg-gradient-to-r from-teal-500/10 to-cyan-500/10 rounded-xl border border-teal-500/20">
                    ...
                    >
                        <div className="p-6 space-y-5">
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        {i === 0 && (
                                            <div className="flex items-center gap-1 px-2 py-1 bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-lg border border-purple-500/30">
                                ...
                                <div className="text-center bg-slate-900/50 p-3 rounded-xl border border-slate-800 min-w-[70px]">
                                    <p className="text-2xl font-black bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                                        {rec.fit_score}%
                                    </p>
                                    <p className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">AI Fit</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 pt-2">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Key Benefits</p>
                                <div className="space-y-1.5">
                                    {rec.why.slice(0, 2).map((reason, idx) => (
                            ...
                            <div className="flex flex-wrap gap-1.5 pt-2">
                                {rec.tags.map((tag, idx) => (
                                    <span key={idx} className="px-2 py-1 bg-slate-900/50 rounded text-[10px] font-medium text-slate-400 border border-slate-800">
                                        {tag}
                                    </span>
                                ))}
                            </div>
            ...
            {/* Disclaimer */}
            <div className="bg-gradient-to-br from-slate-900/30 to-slate-800/30 border border-slate-800/50 rounded-2xl p-6">
                <div className="flex items-start gap-3">
                    <Info className="text-purple-500 flex-shrink-0 mt-0.5" size={16} />
                    <div>
                        <p className="text-xs text-slate-400">
                            <strong className="text-slate-300">Disclaimer:</strong> AI-generated strategies use machine learning for market analysis. Past performance doesn't guarantee future results. Trading involves risk. Consult financial advisors before investing. Strategies update based on market conditions.
                        </p>
                    </div>
                </div>
            </div>
