        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-xl">
                ...
                <div className="grid grid-cols-1 gap-3">
                    {Object.entries(results.symbol_stats).map(([symbol, stats]: any) => (
                        <div key={symbol} className="flex items-center justify-between p-4 bg-slate-800/40 border border-slate-700/50 rounded-xl hover:bg-slate-800/60 transition-all">
                            ...
                            <div className="flex items-center space-x-8">
                                <div className="text-right">
                                    ...
                                    <p className={`font-bold ${stats.total_profit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                                        {formatCurrency(stats.total_profit)}
                                    </p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-slate-500 font-medium mb-0.5">Trades</p>
                                    <p className="font-bold text-slate-200">{stats.num_trades}</p>
                                </div>
                                <div className="text-right">
            ...
            <div className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 shadow-xl">
                ...
                <ResponsiveContainer width="100%" height={320}>
                    <AreaChart data={results.equity_curve}>
                        ...
                        />
                        <Area type="monotone" dataKey="equity" stroke="#8b5cf6" strokeWidth={2} fillOpacity={1} fill="url(#colorMultiEquity)" />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border border-slate-800/50 rounded-2xl overflow-hidden shadow-2xl">
                <div className="px-6 py-5 border-b border-slate-800/50 flex justify-between items-center bg-white/[0.02]">
                    <div>
                        <h3 className="text-lg font-bold text-slate-100 tracking-tight">Trade Ledger</h3>
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] mt-1">Transaction History & Execution Details</p>
                    </div>
                                    ...
                                    <tr key={idx} className="hover:bg-white/[0.02] transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center space-x-3">
                                                <span className="w-8 h-8 rounded bg-slate-800 flex items-center justify-center font-mono text-[10px] font-bold text-violet-400 border border-slate-700">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded text-[9px] font-black uppercase tracking-tighter ${trade.side === 'LONG' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
                                                }`}>
                                                {trade.side}
                                            </span>
                                        </td>
                ...
                <div className="px-6 py-4 bg-white/[0.01] border-t border-slate-800/50 flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="flex items-center space-x-1">
                            ...
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Average Winner: {formatCurrency(results.avg_win || 0)}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                            <div className="w-2 h-2 rounded-full bg-red-500" />
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Average Loser: {formatCurrency(results.avg_loss || 0)}</span>
                        </div>
                    </div>
                </div>
            </div>
