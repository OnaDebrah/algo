...
import { BrokerType, EngineStatus, ExecutionOrder, LiveStatus } from '@/types/api.types';
    ...
    const handleConnect = async () => {
        ...
        try {
            if (isConnected) {
                ...
                await live.connect({ broker: activeBroker });
            }
            await fetchStatus();
        } catch (err) {
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800/50 rounded-2xl p-6">
                        <div className="flex items-center gap-2 mb-4">
                            <Wallet className="text-emerald-500" size={16} />
                            <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Equity Snapshot</h4>
                        </div>
                        <div className="space-y-4">
                            <div>
                    ...
                    <div className="bg-slate-900/50 border border-slate-800/50 rounded-2xl p-6">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">
                                    Status: <span className={`font-bold ${engineStatus === EngineStatus.RUNNING ? 'text-emerald-400' : 'text-slate-400'}`}>{engineStatus.toUpperCase()}</span>
                                </p>
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Live Order Bliss</h4>
                                <RefreshCcw
                                    size={14}
                                            ...
                                            <tr key={order.id} className="group hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="py-4">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-[10px] font-bold text-slate-300 uppercase tracking-tight">{order.status}</span>
                                                    </div>
                                                </td>
